    ...
    <div id="relatedPost" class="bm">
      ...
      <div class="bmc">
		...
		<?php foreach((array)Bagecms::getList('post','index_news',array('where'=>"status_is='Y' AND catalog_id={$bagecmsShow['catalog_id']} AND id!={$bagecmsShow['id']}",'order'=>'id DESC', 'limit'=>8)) as $newsKey=>$newsRow):?>
          <li>• <a href="<?php if($newsRow['redirect_url']):?><?php echo XUtils::convertHttp($newsRow['redirect_url'])?><?php else:?><?php echo $this->createUrl('post/show',array('id'=>$newsRow['id']))?><?php endif?>" target="_blank"><?php echo $newsRow['title']?></a></li>
		<?php endforeach?> 
        </ul>
      </div>
    </div>
