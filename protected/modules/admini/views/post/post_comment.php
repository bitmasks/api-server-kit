...
<div id="contentHeader">
  ...
  <div class="searchArea">
    <ul class="action left" >
      <li><a href="<?php echo $this->createUrl('create')?>" class="actionBtn"><span>新建内容</span></a></li>
    </ul>
  ...
  <form method="post" action="<?php echo $this->createUrl('batch')?>" name="cpform" >
    ...
    <tr class="tb_list" <?php if($row->status_is=='N'):?>style=" background:#F0F7FC"<?php endif?>>
      <td ><input type="checkbox" name="id[]" value="<?php echo $row->id?>">
        <?php echo $row->id?></td>
      <td ><?php echo CHtml::encode($row->nickname) ?></td>
      <td ><?php echo $row->post->title ?></td>
      <td ><?php echo nl2br(CHtml::encode($row->content)) ?></td>
